...
{
	assert(aX < mWidth && aY < mHeight);  // Ensure the coordinates are within bounds.

	// Get the linear index using the get_linear_index method. This is the index for the first color component (R).
	Index linearIndex = get_linear_index(aX,mHeight - aY) * 4;  
